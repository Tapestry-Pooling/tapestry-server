...
class COMP:
  ...
  # Counts number of tests which are positive for each sample
  def count_num_positive_tests(self, infections):
    num_positive_tests = np.zeros(self.n)
    for person in range(self.n):
      num_infected = 0
      for test in range(self.t):
        if self.M[person, test] == 1 and infections[test] == 1:
          num_infected += 1

      num_positive_tests[person] = num_infected
    return num_positive_tests
